...
methodology. Consumes (from the sibling ../1-6_csv/ folder, calib_-prefixed):
  - calib_{fp32,fp16,int8,modelopt_int8,modelopt_int8_excl_cv4,
    modelopt_int8_excl_cv4_fp32}.csv -- deployed-model TensorRT inference,
    originally produced on the Nano.
  - calib_yolo11x_fp32.csv -- reference-curve inference, originally produced
    on the PC via plain onnxruntime (yolo11x is never quantized/deployed so
    this didn't need the Nano).

(The upstream frame-generation/inference scripts that produced these CSVs
from raw footage aren't bundled here -- this repo only ships the final
analysis/chart scripts, matching every other N-M folder's convention.)

Writes the promoted Fig 1-6-2 (pdf/png/svg) into the shared ../charts/
folder -- this is now the canonical 1-6-2 figure (the old synthetic-
calibrated version was retired, not just moved). calib_final_miss_rate_by_variant.csv
is written back into RESULTS_DIR (../1-6_csv/), alongside the data it was
computed from.
...
RESULTS_DIR = EXPERIMENT_ROOT / "1-6_csv"
...
def main():
    x11_path = RESULTS_DIR / "calib_yolo11x_fp32.csv"
    if not x11_path.exists():
        raise SystemExit(f"expected {x11_path} -- should be bundled in 1-6_csv/")
    ...
    for variant in DEPLOYED_VARIANTS:
        v_path = RESULTS_DIR / f"calib_{variant}.csv"
    ...
    if all_summaries:
        ...
        combined.to_csv(RESULTS_DIR / "calib_final_miss_rate_by_variant.csv", index=False)
        print(f"Saved combined results to {RESULTS_DIR / 'calib_final_miss_rate_by_variant.csv'}")
...
def make_fig12(combined: pd.DataFrame, x11_curve: pd.DataFrame):
    ...
    out_base = CHARTS_DIR / "1-6-2_R_value_raw_vs_corrected_vs_distance"
    fig.savefig(f"{out_base}.pdf", bbox_inches="tight")
    fig.savefig(f"{out_base}.png", dpi=130, bbox_inches="tight")
    fig.savefig(f"{out_base}.svg", bbox_inches="tight")
    plt.close(fig)
    print(f"Saved Fig 1-6-2 (v3 methodology) to {out_base}.pdf/.png/.svg")
