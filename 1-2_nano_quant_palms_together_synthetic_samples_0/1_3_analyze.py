#!/usr/bin/env python3
"""
1_3_analyze.py -- 1-3 (palms-together synthetic dataset) analysis.

Same methodology as 1_2_analyze.py (see ../common/pose_quant_analysis.py
for the full writeup: per-image aggregation, character-identity recovery
via arrangement.json, pooled within-character R statistics, the 0.80
shoulder-ratio exclusion filter). This script has NOT yet been reconciled
against a published Notion "1-3 (新)" section -- that rebuild is still
pending -- but produces the same five tables (a)-(e) as 1-2 so the
palms-together rebuild can reuse it directly once it's time to do that.

Run: python 1_3_analyze.py
"""
import sys
