...
the per-detection CSVs in 1-5_csv.7z. Extract it into a sibling folder
"1-5_csv" (per-model subfolders, each with synth_fp32.csv /
synth_fp16.csv / synth_modelopt_int8_full.csv / synth_modelopt_int8_mixed.csv)
...
DATA_DIR = Path(__file__).resolve().parent.parent / "1-5_csv"
...
def main():
    ...
    if not YOLO11X_FP32_CSV.exists():
        ...
        return
    if not (DATA_DIR / MODELS[0][0] / "synth_fp16.csv").exists():
        print(f"\n[SKIP Fig 1-5-7] FP16 CSVs not found under {DATA_DIR} -- extract 1-5_csv.7z here first")
        print("DONE (1-5-1 through 1-5-6 only)")
        return
    ...
    for model, arch, tier in MODELS:
        model_dir = DATA_DIR / model
        fp32_df = pd.read_csv(model_dir / "synth_fp32.csv")
        fp16_df = pd.read_csv(model_dir / "synth_fp16.csv")
